...
public class CrateMenu extends AbstractContainerMenu {
    ...
    public SimpleContainerNBT interactableTradeSlots;
    public SimpleContainerNBT savedTradeSlots;
    ...
    protected CrateMenu(MenuType type, int id, Inventory inventory, boolean unlimited) {
        this(type, id, inventory, new SimpleContainerNBT(2), new SimpleContainerNBT(2), new CrateStackHandler(), unlimited);
    }

    public CrateMenu(MenuType type, int id, Inventory inventory, SimpleContainerNBT interaction, SimpleContainerNBT trade, CrateStackHandler stock, boolean ul) {
        super(type, id);
        ...
        this.addSlot(new Slot(interactableTradeSlots, 0, isOwner() ? 10 : 102, isOwner() ? 76 : 21));
        this.addSlot(new Slot(interactableTradeSlots, 1, isOwner() ? 46 : 142, isOwner() ? 76 : 21) {
            @Override
    ...
    public void tryMovePaymentToInteraction() {
        ...
        ItemStack itemstack = this.interactableTradeSlots.getItem(0);
        if (!itemstack.isEmpty()) {
            if (!this.moveItemStackTo(itemstack, Constants.PLAYERSTARTSLOT, Constants.PLAYERENDSLOT, false)) {
                return;
            }
            this.interactableTradeSlots.setItem(0, itemstack);
        }

        if (this.interactableTradeSlots.getItem(0).isEmpty()) {
            ItemStack give = this.savedTradeSlots.getItem(0);
        ...
        if (!give.isEmpty()) {
            for (int i = 34; i < (34 + (9 * 4)); ++i) {
                ...
                if (!stackinSlot.isEmpty() && ItemStack.isSameItemSameTags(give, stackinSlot)) {
                    ItemStack paymentSlot = this.interactableTradeSlots.getItem(0);
                    ...
                    copy.setCount(l);
                    this.interactableTradeSlots.setItem(0, copy);
    ...
    private void returnItemToInventory(Player player, int index) {
        player.getInventory().placeItemBackInInventory(this.interactableTradeSlots.getItem(index));
        this.interactableTradeSlots.setItem(index, ItemStack.EMPTY);
    }
    ...
    protected void updateSellItem() {
        ...
        } else if (!interactableTradeSlots.getItem(1).isEmpty()) {
            //move out slot back to crate stock
            moveItemStackTo(interactableTradeSlots.getItem(1), Constants.CRATESTARTSLOT, Constants.CRATEENDSLOT, false);
        }
    }

    protected ItemStack pickUpPayment() {
        ItemStack original = crateStock.getItem(Constants.TOTALCRATESTOCKLOTS); //do not modify originals !
        ...
        if (amount > 0 && original.hasTag()) { //Redundant double check, but better safe then sorry
            ...
            if (updatedAmount <= 0) {
                crateStock.setItem(Constants.TOTALCRATESTOCKLOTS, ItemStack.EMPTY);
            } else {
                ItemStack prepUpdate = original.copy();
                prepUpdate.getTag().putInt(Constants.TAGSTOCK, updatedAmount);
                crateStock.setItem(Constants.TOTALCRATESTOCKLOTS, prepUpdate);
            }
            return prepPickup;
        }
        return ItemStack.EMPTY;
            ...
            if (isUnlimitedShop)
                ...
                for (int i = Constants.CRATESTARTSLOT; i < Constants.CRATEENDSLOT; ++i) {
                    ...
                    if (!stackinSlot.isEmpty() && ItemStack.isSame(get, stackinSlot)) {
                        ...
                        stackinSlot.shrink(k);
                        copy.setCount(l);
                        this.interactableTradeSlots.setItem(1, copy);
                        if (l >= get.getCount()) {
                            break;
                        }
                    }
